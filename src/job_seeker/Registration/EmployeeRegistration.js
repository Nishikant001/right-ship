...
const EmployeeRegistration = () => {
  ...
  const loadFormData = () => {
    const savedFormData = localStorage.getItem('employeeRegistrationFormData');
    return savedFormData ? JSON.parse(savedFormData) : {
      firstName: '',
      lastName: '',
      email: isEmail ? contactInfo : '',
      mobile_no: isEmail ? '' : contactInfo,
      whatsappNumber: '',
      gender: '',
      country: '',
      dob: '',
  ...
  useEffect(() => {
    localStorage.setItem('employeeRegistrationFormData', JSON.stringify(formData));
  }, [formData]);

  useEffect(() => {
    // Replace the history state to prevent going back to OTP verification
    window.history.replaceState(null, '', '/employee-registration');

    const handlePopState = () => {
      // Stay on the registration page if the user tries to navigate back
      navigate('/employee-registration', { replace: true });
    };

    // Listen for the popstate event (triggered when navigating with back/forward)
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!loading && !error) {
        ...
        localStorage.removeItem('employeeRegistrationFormData'); // Clear data after successful registration
        navigate('/jobs');
      }
    } catch (err) {
